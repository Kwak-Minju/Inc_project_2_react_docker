...
const BoardViewContent = ({boardId}) =>{
    ...
    // 게시판 데이터 가져오기
    useEffect(() => {
        axios.post(flask_uri + `/boardDetail`, {id : boardId}, { headers: { 'Content-Type': 'application/json' } })
        .then(response => {
    ...
    const handlerEditFinish= () =>{
        // 수정완료 후 데이터 변경
        axios.post( flask_uri + `/boardEdit`, {title : title, content : content, boardId: boardId }, { headers: { 'Content-Type': 'application/json' } })
        .then(response => {
    ...
    const handlerDel = () => {
        if (sessionStorage.getItem('userId') == boardData[0].userId) {
            // 게시물 삭제
            axios.post( flask_uri + `/boardDelete`, {boardId: boardId}, { headers: { 'Content-Type': 'application/json' } })
            .then(response => {
