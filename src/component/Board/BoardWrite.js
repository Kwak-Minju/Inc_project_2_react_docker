...
const BoardWrite = () => {
    ...
    useEffect(() => {
        axios.post(flask_uri + `/checkid`,{userId : sessionStorage.getItem('userId')} )
        .then(responce => {
    ...
    const postData = () => {
        if (title === '') {
            ...
            axios.post(flask_uri + `/boardWrite`, {title : title, location : location, content : content, userId : sessionStorage.getItem('userId') }, { headers: { 'Content-Type': 'application/json' } })
            .then(response => {
